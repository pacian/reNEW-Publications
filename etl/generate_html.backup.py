...
def generate_html():
    ...
    full_data.sort(key=parse_date, reverse=True)

    for pub in full_data:
        try:
            dt = parse_date(pub)
    ...
    year_counts = Counter(pub["year_only"] for pub in full_data if pub["year_only"])
    ...
    <body>
      ...
      <div class="container">
        ...
        <select id="yearFilter" onchange="filterAll()">
          <option value="all">All years ({{ full_data | length }})</option>
          {% for y in years %}
        ...
        <div class="match-count" id="matchCount">Showing {{ full_data | length }} of {{ full_data | length }} results</div>
